...
model_nlp = AutoModelForSequenceClassification.from_pretrained('kit-nlp/bert-base-japanese-sentiment-irony') 
tokenizer = BertJapaneseTokenizer.from_pretrained('cl-tohoku/bert-base-japanese-whole-word-masking')
nlp = pipeline("sentiment-analysis", model=model_nlp, tokenizer=tokenizer)
...
custom_lm = TransformerLanguageModel.from_pretrained('music-ckps/', 'checkpoint_best.pt', tokenizer='space', batch_size=8192)
...
def save_demo(notes_str, select_chords, name, lang, sentence, word_counter):
    ...
    mf.addTempo(melody_track, time, int(tempo))
    mf.addTempo(chord_track, time, int(tempo))
            ...
            if len(chord_time) > 0:
                ...
                for i in range(insert_num):
                    for shift in _CHORD_KIND_PITCHES[cd_type]:
                        mf.addNote(chord_track, channel, 48 + root + shift, chord_time[-1] + 1, 2, 75)
                    chord_time.append(chord_time[-1] + 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--debug', default='', help='Output composition details')
    ...
    parser.add_argument('--tempo', default=120)
    ...
    with open(lyrics_path) as lyrics_file, open(chord_path) as chord_file:
        ...
        for lyrics, chord in zip(lyrics_corpus, chord_corpus):
            ...
            #lang = chord[:2]
            lang = lyrics[:2]
            assert lang in ['zh', 'en', 'ja']  # Note that ROC is not language-sensitive, you can extend this.
            ...
            if lang == 'zh':
                sentence = lyrics[2:].strip().split(' ')  # The list of lyric sentences
                name = sentence[0]
                ...
                sentence = lyrics[2:].strip().split('[sep]')
                name = sentence[0]
            ...
            for i in range(len(sentence)):
                if lang == 'zh' or lang == 'ja':
                    ...
                    print('Lyrics: ', lyrics[2:].strip().split('[sep]')[i])
            ...
            save_demo(note_string, select_chords, name, lang,
                      lyrics[2:].strip().split('[sep]') if lang == 'en' else sentence, [len(i) for i in sentence])
